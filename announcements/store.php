...
if($code){
    ...
    if($access_token && $access_token !== NULL){
        ...
        if ($user_id){
            ...
            $data = array("hub.callback" => "https://chukwumaokere.com/twitch/announcements/enroll.php?client_id=$client_id&app_access_token=$app_access_token&access_token=$access_token&user_id=$user_id", "hub.mode" => "subscribe", "hub.topic" => "https://api.twitch.tv/helix/streams?user_id=$user_id", "hub.lease_seconds" => 864000);
...
function saveToDatabase($preferred_name, $access_token, $app_access_token, $client_id, $user_id, $auth_code){
    ...
    $q1 = "INSERT INTO announcement_store (user_id, preferred_name, access_token, app_access_token, client_id) VALUES ('$user_id', '$preferred_name', '$access_token', '$app_access_token', '$client_id')";
    $add1 = $db->query($q1);
    if ($add1){
        return true;    
    }else{
